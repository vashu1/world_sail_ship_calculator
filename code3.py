...
CELL_SCAN_RADIUS = 1 #3 #TODO
LATTITUDE_NORTH_BOUNDARY = 160
LATTITUDE_SOUTH_BOUNDARY = 20
TOO_BIG_DISTANCE = datetime(year=3000, month=1, day=1)
...
distances = [[(None if y<LATTITUDE_SOUTH_BOUNDARY or y>LATTITUDE_NORTH_BOUNDARY or is_land(x, y) else TOO_BIG_DISTANCE) for y in range(180)] for x in range(360)]
distances[-83+180][10+90] = None  # PANAMA 1degree closure (x==-83 and y==10) or  (x==-82 and y==9) or  (x==-81 and y==9)
distances[-82+180][9+90] = None
distances[-81+180][9+90] = None
#edge = deque([start_point])
edge = []

start_dt = cur_dt = datetime(year=1997, month=5, day=30, hour=12)
heappush(edge, (start_dt, start_point))
distances[start_point[0]][start_point[1]] = start_dt

t = time.time()
while edge:
    #x0, y0 = edge.popleft()
    d0, (x0, y0) = heappop(edge)
    if d0 > distances[x0][y0]:
        continue
    #d0 = distances[x0][y0]
    for scan in range(1, CELL_SCAN_RADIUS+1):  # if there is land n cells nearby, stop scan
        near_land = any([distances[x][y] is None for x, y in neighbours(x0, y0, scan)])
        if not near_land and scan != CELL_SCAN_RADIUS:
            continue
        for x, y in neighbours(x0, y0, 1 if near_land else scan):
            if distances[x][y] and distances[x0][y0] > distances[x][y]:
                continue
            c += 1
            if c % 100 == 0:
                print(c, len([i for i in flatten(distances) if i is not None and i < TOO_BIG_DISTANCE]), len(edge), int(time.time()-t))  # 1411000 49031
            if distances[x][y] is None:  # land
                continue
            if d0 > distances[x][y]:
                continue
            max_dt = (TOO_BIG_DISTANCE if distances[x][y] is None else distances[x][y]) - start_dt
            max_dt = timedelta(days=10) if timedelta(days=10) < max_dt else max_dt
            dt = navigation.run_path(start_dt, ax, m, d0, norm_long(x0), norm_latt(y0), norm_long(x), norm_latt(y), max_dt=max_dt)
            if dt is None:
                continue
            if distances[x][y] > dt:
                distances[x][y] = dt
                heappush(edge, (dt, (x, y)))
        break
...
for _, (x, y) in edge:
    _ = ax.annotate('.', (norm_long(x), norm_latt(y)), xytext=(5, 5), textcoords='offset points')
